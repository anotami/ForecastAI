...
if 'aht_val' not in st.session_state: st.session_state.aht_val = 550.0 # TMO base solicitado
if 'shr_val' not in st.session_state: st.session_state.shr_val = 0.25  # Shrinkage base solicitado

# --- SIDEBAR: PARÁMETROS BPO (PERSISTENTE) ---
with st.sidebar:
...
if st.session_state.step == 1:
    ...
    if fuente == "Simulación Aleatoria":
        ...
        if st.button("Generar Histórico ➡️"):
            # Se inyecta variabilidad desde el data_loader
...
# --- PASO 2: FORECAST & BACKTESTING ---
    ...
    if st.button("🚀 Ejecutar Pronóstico"):
        # Cálculo de periodos estricto al rango solicitado
        dias_totales = (pd.to_datetime(f_fin).date() - df['ds'].min().date()).days
        with st.spinner("Entrenando modelos flexibles..."):
            # run_prophet ahora usa changepoint_prior_scale=0.5
    ...
    if 'current_forecast' in st.session_state:
        ...
        # Lógica de MAPE y Sombreado de Validación
...
# --- PASO 3: STAFFING JERÁRQUICO ---
    ...
    # Lógica de Jerarquía solicitada
    ...
    # Gráfico Maestro con colores corporativos
    fig_s = go.Figure()
    ...
    # Cálculo de Ocupación para auditar Punto 3
    ocupacion = (res_wfm['yhat'] * aht) / (res_wfm['agentes_netos'] * 1800)
    st.metric("Ocupación Promedio Proyectada", f"{ocupacion.mean():.1%}")
